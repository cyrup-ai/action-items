...
mod tests {
    ...
    fn test_lifecycle_state_transitions() {
        ...
        assert!(
            lifecycle
                .transition_to(
                    NotificationState::InteractionReceived,
                    TransitionReason::UserInteraction,
                    None
                )
                .is_err()
        );
    ...
    fn test_retry_policy_backoff() {
        ...
        let delay1 = policy.calculate_next_delay(1);
        let delay2 = policy.calculate_next_delay(2);
        let delay3 = policy.calculate_next_delay(3);
    ...
    fn test_expiration_checking() {
        ...
        // Set short TTL
        lifecycle.expiration.ttl = Some(Duration::from_millis(1));
    ...
    fn test_delivery_progress() {
        ...
        // No platforms - progress based on state
        assert_eq!(lifecycle.delivery_progress(), DeliveryProgress::NotStarted);

        // Add platform states
        lifecycle.update_platform_state(Platform::MacOS, PlatformDeliveryState {
            platform: Platform::MacOS,
            status: PlatformDeliveryStatus::Delivered,
            native_id: Some("test".to_string()),
            attempt_count: 1,
            last_attempt: Some(*DefaultableInstant::now()),
            delivery_latency: Some(Duration::from_millis(100)),
            error_details: None,
        });

        lifecycle.update_platform_state(Platform::Windows, PlatformDeliveryState {
            platform: Platform::Windows,
            status: PlatformDeliveryStatus::Failed("Test error".to_string()),
            native_id: None,
            attempt_count: 2,
            last_attempt: Some(*DefaultableInstant::now()),
        ...
        match lifecycle.delivery_progress() {
            DeliveryProgress::PartiallyComplete { successful, total } => {
                assert_eq!(successful, 1);
                assert_eq!(total, 2);
            },
            _ => panic!("Expected PartiallyComplete"),
        }
    }
}
