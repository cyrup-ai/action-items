...
/// **Viewport width** - Represents scalable unit `0% to 100%` of the root container. `120%` is
...
/// **Viewport Height** - Represents scalable unit `0% to 100%` of the root container. `120%` is
...
/// Streamlined macro that implements all basic math operations for UI unit types
macro_rules! implement_unit_math {
    ($($unit:ident), *) => {
        $(
            // Implement negation
            impl <T: Neg<Output = T>> Neg for $unit<T> {
                type Output = Self;
                fn neg(self) -> Self::Output { $unit(-self.0) }
            }

            // Implement addition
            impl <T: Add<Output = T>> Add for $unit<T> {
                type Output = Self;
                fn add(self, other: Self) -> Self::Output { $unit(self.0 + other.0) }
            }
            ...
            impl <T: Sub<Output = T>> Sub for $unit<T> {
                type Output = Self;
                fn sub(self, other: Self) -> Self::Output { $unit(self.0 - other.0) }
            }
            ...
            impl <T: Mul<Output = T>> Mul for $unit<T> {
                type Output = Self;
                fn mul(self, other: Self) -> Self::Output { $unit(self.0 * other.0) }
            }
            impl <T: MulAssign<T>> MulAssign for $unit<T> {
                fn mul_assign(&mut self, rhs: Self) { self.0 *= rhs.0 }
            }

            // Implement f32 multiplication
            impl <T: Mul<f32, Output = T>> Mul<f32> for $unit<T> {
                type Output = $unit<T>;
                fn mul(self, rhs: f32) -> Self::Output { $unit(self.0 * rhs) }
            }
...
implement_unit_math!(Ab, Rl, Rw, Rh, Em, Vp, Vw, Vh);

/// Implement vector conversions for unit types - more concise and maintainable
macro_rules! implement_unit_vector_conversions {
    ($($unit:ident), *) => {
        $(
            ...
            // Single value to vector conversions (splat)
            impl From<$unit<f32>> for UiValue<Vec2> {
                fn from(val: $unit<f32>) -> UiValue<Vec2> { $unit(Vec2::splat(val.0)).into() }
            }
            impl From<$unit<f32>> for UiValue<Vec3> {
                fn from(val: $unit<f32>) -> UiValue<Vec3> { $unit(Vec3::splat(val.0)).into() }
            }
            impl From<$unit<f32>> for UiValue<Vec4> {
                fn from(val: $unit<f32>) -> UiValue<Vec4> { $unit(Vec4::splat(val.0)).into() }
            }
        )*
    };
}
implement_unit_vector_conversions!(Ab, Rl, Rw, Rh, Em, Vp, Vw, Vh);
...
macro_rules! impl_unit_cross_operations {
    (($unit1:ident, $ufield1:ident),($unit2:ident, $ufield2:ident)) => {
        ...
        impl<T: Sub<Output = T>> Sub<$unit2<T>> for $unit1<T>
        where
            T: Neg<Output = T>,
        {
            type Output = UiValue<T>;
...
impl<A, B> From<(A, B)> for UiValue<Vec2>
where
    A: Into<UiValue<f32>>,
    B: Into<UiValue<f32>>,
{
...
impl<A, B, C> From<(A, B, C)> for UiValue<Vec3>
where
    A: Into<UiValue<f32>>,
    B: Into<UiValue<f32>>,
    C: Into<UiValue<f32>>,
...
impl<A, B, C, D> From<(A, B, C, D)> for UiValue<Vec4>
where
    A: Into<UiValue<f32>>,
    B: Into<UiValue<f32>>,
    C: Into<UiValue<f32>>,
...
impl UiValueEvaluate<f32> for UiValue<f32> {
    fn evaluate(
        ...
        if let Some(v) = self.ab {
            out += v * absolute_scale
        }
        if let Some(v) = self.rl {
            out += (v / 100.0) * parent_size
        }
...
impl UiValueEvaluate<Vec2> for UiValue<Vec2> {
    fn evaluate(
        ...
        if let Some(v) = self.rw {
            out += (v / 100.0) * parent_size.x
        }
        if let Some(v) = self.rh {
            out += (v / 100.0) * parent_size.y
        }
        if let Some(v) = self.em {
            out += v * font_size
        }
        ...
        if let Some(v) = self.vw {
            out += (v / 100.0) * viewport_size.x
        }
        if let Some(v) = self.vh {
            out += (v / 100.0) * viewport_size.y
        }
        out
    }
}
...
impl NiceDisplay for Anchor {
    fn to_nicestr(&self) -> String {
        match self {
            ...
            Anchor::Custom(point) => format!(
                "({}, {})",
                format!("{}", point.x).green(),
                format!("{}", point.y).green()
            ),
        }
    }
}
...
impl NiceDisplay for UiValue<Vec2> {
    fn to_nicestr(&self) -> String {
        ...
        if tx.is_empty() {
            tx = format!("{}", "0".bright_blue());
        };
        if ty.is_empty() {
            ty = format!("{}", "0".bright_blue());
        };
        format!("({tx}, {ty})")
    }
}
