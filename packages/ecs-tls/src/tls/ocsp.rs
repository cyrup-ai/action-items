...
// Module-level constants (validated once at startup)
static SHA256_OID: Lazy<der::asn1::ObjectIdentifier> = Lazy::new(|| {
    der::asn1::ObjectIdentifier::new("2.16.840.1.101.3.4.2.1")
        .expect("SHA-256 OID is statically known to be valid per RFC 6234")
});

static OCSP_NONCE_OID: Lazy<der::asn1::ObjectIdentifier> = Lazy::new(|| {
    der::asn1::ObjectIdentifier::new("1.3.6.1.5.5.7.48.1.2")
        .expect("OCSP Nonce OID is statically known to be valid per RFC 6960")
});
...
impl OcspCache {
    pub fn new() -> Result<Self, TlsError> {
        let http_client = Client::builder()
            ...
            .build()
            .map_err(|e| TlsError::HttpClientInitialization(e.to_string()))?;
    ...
    pub async fn has_cached_entry(&self, cache_key: &str) -> bool {
        if let Some(entry) = self.get_cached_status(cache_key) {
            !Self::is_cache_expired(&entry)
        } else {
            false
        }
    }
    ...
    fn is_cache_expired(entry: &OcspCacheEntry) -> bool {
        ...
        match now.duration_since(entry.cached_at) {
            ...
            Err(_) => {
                tracing::warn!("OCSP cache entry has future timestamp, treating as expired");
                true // If cached_at is in the future, consider expired for security
            },
        }
    }
    ...
    fn parse_ocsp_response(
        ...
        if let Some(nonce_ext) = basic_response
            ...
            .response_extensions
            .as_ref()
            .and_then(|exts| exts.iter().find(|ext| ext.extn_id == *OCSP_NONCE_OID))
