...
/// Type alias for complex certificate parsing result
type CertificateParsingResult = (
    Vec<String>,                 // SAN DNS names
    Vec<std::net::IpAddr>,      // SAN IP addresses  
    bool,                       // Is CA flag
    Vec<String>,                // Key usage extensions
    SystemTime,                 // Not before time
    SystemTime,                 // Not after time
);
...
pub fn extract_certificate_details(
    cert: &X509CertCert,
) -> Result<CertificateParsingResult, TlsError> {
    // Extract SANs
                                        ...
                                        if let Ok(name_header) = reader.peek_header() {
                                            match name_header.tag.number() {
                                                ...
                                                _ => {
                                                    ...
                                                    // (rfc822Name, x400Address, directoryName,
...
pub fn parse_certificate_from_pem_internal(pem_data: &str) -> Result<ParsedCertificate, TlsError> {
    ...
    // Iterate through all extensions to find Authority Information Access and CRL Distribution
    // Points
    if let Some(extensions) = &cert.tbs_certificate.extensions {
        for ext in extensions.iter() {
