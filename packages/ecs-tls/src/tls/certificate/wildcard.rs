...
/// Creates a non-expiring certificate for *.cyrup.dev with SAN entries for *.cyrup.ai,
/// *.cyrup.cloud, *.cyrup.pro
#[allow(dead_code)] // SweetMCP integration feature - not yet activated
pub(crate) async fn generate_wildcard_certificate(xdg_config_home: &Path) -> Result<(), TlsError> {
    ...
    if wildcard_cert_path.exists() {
        if validate_existing_wildcard_cert(&wildcard_cert_path)
            .await
            .is_ok()
        {
            info!(
...
#[allow(dead_code)] // Helper for wildcard certificate feature - used by generate_wildcard_certificate
pub(crate) async fn validate_existing_wildcard_cert(cert_path: &Path) -> Result<(), TlsError> {
