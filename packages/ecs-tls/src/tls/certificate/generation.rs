...
pub async fn new(
    ...
    CertificateDer<'static>,
    PrivatePkcs8KeyDer<'static>,
    crate::tls::ocsp::OcspCache,
    crate::tls::crl_cache::CrlCache,
)> {
    ...
    let ocsp_cache = crate::tls::ocsp::OcspCache::new()
        .map_err(|e| anyhow::anyhow!("Failed to initialize OCSP cache: {}", e))?;
    let crl_cache = crate::tls::crl_cache::CrlCache::new()
        .map_err(|e| anyhow::anyhow!("Failed to initialize CRL cache: {}", e))?;
...
async fn load_ca(
    ...
    let san_names: Result<Vec<_>, _> = parsed_cert
        .san_dns_names
        .iter()
    ...
    Ok((CertificateDer::from(cert_der.to_vec()), key_der, issuer))
}
