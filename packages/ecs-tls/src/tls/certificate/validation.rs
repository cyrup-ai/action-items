...
pub async fn validate_certificate_chain(
    ...
    for i in 0..chain_certs.len() - 1 {
        ...
        if cert.issuer != issuer.subject {
            return Err(TlsError::CertificateValidation(format!(
                "Certificate chain broken: certificate {} issuer does not match certificate {} \
                 subject",
                i,
                i + 1
            )));
    ...
    let root_cert = chain_certs.last().ok_or_else(|| {
        TlsError::CertificateValidation("Certificate chain is empty, cannot validate".to_string())
    })?;
...
fn parse_certificate_from_der(der_data: &[u8]) -> Result<ParsedCertificate, TlsError> {
    // Convert DER to PEM using existing pattern from authority.rs
    use base64::Engine;
    ...
    // Use existing parser
    super::parser::parse_certificate_from_pem_internal(&pem)
}

/// Verify peer certificate with comprehensive revocation checking (OCSP + CRL + Chain)
#[allow(dead_code)] // Advanced TLS validation function - called by tls_config.rs comprehensive verification
pub(crate) async fn verify_peer_certificate_comprehensive(
